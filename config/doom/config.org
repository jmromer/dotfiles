...
(use-package aider
  :config
  (setq aider-args '("--model"  "gpt-4o" "--weak-model" "o3-mini")))
#+end_src
* completion
...
#+begin_src elisp
(defun +preprocess-markdown-for-mathjax (markdown-text)
  "Preprocess markdown text to ensure LaTeX delimiters work with MathJax."
...
(defun +markdown-preview-gfm ()
  "Preview the current markdown file with GitHub-flavored markdown and MathJax."
  (interactive)
  (let* ((filename (buffer-file-name))
         (markdown-content (buffer-string))
         (preview-file (concat temporary-file-directory "markdown-preview.html"))
         ;; Pre-process the markdown content
         (processed-content (+preprocess-markdown-for-mathjax markdown-content))
         ;; Use pandoc to convert markdown to HTML with GitHub styling
         (html-content
          (with-temp-buffer
            ...
            (shell-command-on-region (point-min) (point-max)
                                    ...
                                    (current-buffer) t)
            (buffer-string)))
         ;; Full HTML document with MathJax
         (full-html
          (concat "<!DOCTYPE html>
<html>
<head>
...
blockquote { color: #6a737d; border-left: 4px solid #dfe2e5; padding-left: 16px; margin-left: 0; }
</style>
<!-- MathJax -->
...
(after! markdown-mode
  (map! :map markdown-mode-map
        :localleader
        :desc "Preview GFM & MathJax" "p" #'+markdown-preview-gfm
        :desc "markdown-preview" "P" #'markdown-preview))
#+end_src

#+begin_src elisp
;; Define the enhanced HTML template with MathJax 3
(defvar my-markdown-preview-gfm-template-mathjax3
  "<!DOCTYPE html>
<html>
<head>
<meta charset=\"utf-8\">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  marked.setOptions({
    ...
    smartLists: true,
    headerIds: true,
    escape: false  // Disable escaping to preserve LaTeX expressions
  });
...
<body>
<div id=\"content\"></div>
<script type=\"text/plain\" id=\"raw-markdown\" style=\"display: none;\">
%s
</script>
</body>
</html>"
  "Template using MathJax 3 with GitHub-flavored markdown.")

;; Function to use the MathJax 3 template
(defun my-markdown-preview-gfm-mathjax3 ()
  "Preview the current markdown file with GitHub-flavored markdown and MathJax 3."
  (interactive)
  (let* ((filename (buffer-file-name))
         (markdown-content (buffer-string))
         (preview-file (concat temporary-file-directory "markdown-preview.html"))
         (html-template (format my-markdown-preview-gfm-template-mathjax3
                                ""
                                markdown-content)))
    ;; Write the HTML to a temporary file
    (with-temp-file preview-file
...
(after! markdown-mode
  (map! :map markdown-mode-map
        :localleader
        :desc "Preview with GFM (MathJax 3)" "M" #'my-markdown-preview-gfm-mathjax3))
