...
(map! :map LaTeX-mode-map
      ...
      :ni "s-<return>" #'+open-latex-doc
      :ni "s-S-<return>" #'+open-latex-doc-in-skim
      :ni "s-s" #'+save-and-compile-latex-doc)
#+end_src
...
#+begin_src elisp
(defun +find-associated-latex-doc (&optional filetype docs-dir-name)
  (interactive)
  (let* ((filetype (or filetype "tex"))
         (docs-dir-name (or docs-dir-name "docs"))
         ...
         (history (mapcar #'file-name-sans-extension (directory-files default-directory nil (format "\.%s" filetype)))))
    (if (file-exists-p (format "%s.%s" target-file-base filetype))
                        target-file-base
                      (string-trim (read-from-minibuffer "open doc: " target-file-base nil nil 'history)))))
#+end_src

*** Compile

#+begin_src elisp
(defun +save-and-compile-latex-doc ()
  (interactive)
  (let ((shell-command-prompt-show-cwd t)
        (file-name (+find-associated-latex-doc))
        (default-directory (concat (projectile-project-root) "docs")))
    (progn
      (save-buffer)
      (+async-shell-command-below
       (format "xelatex -interaction=nonstopmode -halt-on-error %s.tex" file-name file-name)
       nil
       0.25))))
#+end_src
...
#+begin_src elisp
(defun +open-latex-doc (arg)
  (interactive "P")
  (let ((file-name (+find-associated-latex-doc "pdf"))
        (default-directory (concat (projectile-project-root) "docs")))
    (if arg
...
Enable superword mode globally
