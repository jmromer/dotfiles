...
(defun +LaTeX-evaluate-selection ()
  ...
  (let* ((selected-text (string-trim (buffer-substring (mark) (point))))
         (replaced-text (replace-regexp-in-string "\\\\frac{\\([^}]+\\)}{\\([^}]+\\)}" "(\\1/\\2)" selected-text))
         ...
         (replaced-text (replace-regexp-in-string "\\\\cdot" "*" replaced-text))
  ...
  (let* ((filetype (or filetype "tex"))
         ...
         (history
          (mapcar #'file-name-sans-extension
                  (cl-remove-if-not
                   (lambda (fn) (message fn) (string-suffix-p ".tex" fn))
