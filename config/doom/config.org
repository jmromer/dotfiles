...
** =markdown-preview=: gfm-ish live-ish preview
...
(defun +workspace/clone ()
  "Clone the current workspace (naming it similarly)
and insert the new workspace immediately to the right of the current one."
  (interactive)
  (let* ((curr-name (+workspace-current-name))
         (next-name (+workspace--increment-name curr-name)))
    ;; Create the new workspace; assume it becomes the current workspace.
    (+workspace/new next-name t)
    (let* ((names (+workspace-list-names))
           (orig-index (cl-position curr-name names :test #'equal))
           (new-index (cl-position next-name names :test #'equal))
           (swap-count (- new-index (1+ orig-index))))
      (when (> swap-count 0)
        ;; Swap left the new workspace by the required count.
        (+workspace/swap-left swap-count))
      (+workspace-switch curr-name)
      (+workspace/switch-right 1))))
#+end_src
