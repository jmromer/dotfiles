...
(map! :leader
      :prefix "t"
      :desc "word count" "w" #'+enable-continuous-word-count)
#+end_src
...
#+begin_src elisp
(defun +compile-pdflatex-pset ()
  (interactive)
  (let* ((default-directory (concat (projectile-project-root) "psets"))
         (history (mapcar #'file-name-sans-extension (directory-files default-directory nil "\.tex$")))
         (file-name (string-trim (read-from-minibuffer "problem set: " "pset1" nil nil '(history . 1)))))
    (progn
      (message (format "Compiling %s.tex..." file-name))
      (call-process-shell-command (format "pdflatex %s.tex" file-name)))))
#+end_src

#+begin_src elisp
(map! :map LaTeX-mode-map "s-C-t" #'+compile-pdflatex-pset)
#+end_src

#+begin_src elisp
(defun +add-TeX-command-list-entries ()
  (interactive)
  (add-to-list 'TeX-command-list
               '("pdfLaTeX (pset)" "(+compile-pdflatex-pset)"
                 TeX-run-function nil t
                 :help "Compile the current problem set")))
(add-hook! LaTeX-mode #'+add-TeX-command-list-entries)
#+end_src
...
Disable autoformatting until we can figure out what the hell is going on.
...
#+begin_src elisp
(defun rails--parse-controller-alt-files ()
  (let ((path (buffer-path-dwim)))
    (if (string-match (rx (group (or "app" "spec")) "/"
                          (group (or "controllers" "requests")) "/"
                          (group (1+ anything)) "_"
                          (group (or "controller" "spec" "controller_spec" "request_spec")) "."
                          (group "rb"))
                      path)
        (let ((dir (match-string 1 path))
              (subdir (match-string 2 path))
              (file (s-replace "_controller" "" (match-string 3 path)))
              (type (match-string 4 path))
              (ext (match-string 5 path)))
          (list `(dir . ,dir) `(subdir . ,subdir) `(file . ,file) `(type . ,type) `(ext . ,ext))))))
...
Look for a controller spec if there's no request spec."
  (let* ((comps (rails--parse-controller-alt-files))
         (file-type (alist-get 'dir comps))
         (file-name (alist-get 'file comps))
         (implementation (concat "app/controllers/" file-name  "_controller.rb"))
         (request-spec (concat "spec/requests/" file-name "_spec.rb"))
         (controller-spec (concat "spec/controllers/" file-name "_controller_spec.rb"))
         (request-spec-available-p (file-exists-p (concat (projectile-project-root) request-spec))))
  (if (equal file-type "spec")
      (list :impl implementation)
    (list :test (if request-spec-available-p request-spec controller-spec)
