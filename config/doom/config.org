...
(defun browser-open-at-point (use-new-session)
  (interactive "P")
  (if-let (url (browser-open--ensure-url
                (browser-open--get-url-at-point)))
...
(defun yank-buffer-path-dwim (abspath-p)
  "Yank the path of the current buffer's file. (If `buffer-file-name' isn't set, uses `default-directory'.)
Abbreviate the path: If in a project, relative to project root; otherwise to the tilde-abbreviated user root.
Provide an absolute path if teh prefix argument ABSPATH-P is provided."
  (interactive "P")
  (let ((yanked-path
