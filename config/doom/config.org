...
- [[#magit][magit]]
- [[#markdown][markdown]]
- [[#markdown-gfm-live-preview][markdown gfm live preview]]
- [[#org][org]]
...
(map! :leader
      :prefix ("w" . "window")
      :desc "split below & follow"  "s"  #'+evil/window-split-and-follow
      :desc "split below"           "S"  #'evil-window-split
      :desc "split right & follow"  "v"  #'+evil/window-vsplit-and-follow
...
(defun set-markdown-keymaps ()
  ...
  (map! :localleader
        ...
        :desc "open"            "o"  #'markdown-open
        :desc "preview"         "p"  #'markdown-gfm-live-preview
        :prefix ("i" . "insert")
        :desc "header"          "h"  #'markdown-insert-header-setext-dwim
        :desc "header (atx)"    "H"  #'markdown-insert-header-dwim
        :desc "header 1"        "1"  #'markdown-insert-header-setext-1
        :desc "header 2"        "2"  #'markdown-insert-header-setext-2
        :desc "header 3"        "3"  #'markdown-insert-header-atx-3
        :desc "header 4"        "4"  #'markdown-insert-header-atx-4
        :desc "header 5"        "5"  #'markdown-insert-header-atx-5
        :desc "header 6"        "6"  #'markdown-insert-header-atx-6
        :desc "bold"            "b"  #'markdown-insert-bold
        :desc "code"            "c"  #'markdown-insert-code
        :desc "code (gfm)"      "C"  #'markdown-insert-gfm-code-block
        ...
        :desc "pre block"       "P"  #'markdown-pre-region
        :desc "quote"           "q"  #'markdown-insert-blockquote
        :desc "quote block"     "Q"  #'markdown-blockquote-region
        ...
        :desc "checkbox (gfm)"  "["  #'markdown-insert-gfm-checkbox
        :prefix ("t" . "table")
        :desc "sort lines"      "s"  #'markdown-table-sort-lines
        :desc "convert region"  "v"  #'markdown-table-convert-region
        :desc "transpose"       "t"  #'markdown-table-transpose
        :desc "row delete"      "R"  #'markdown-table-delete-row
        :desc "row insert"      "r"  #'markdown-table-insert-row
        :desc "column delete"   "C"  #'markdown-table-delete-column
        :desc "column insert"   "c"  #'markdown-table-insert-column))

(add-hook! markdown-mode #'set-markdown-keymaps)
#+end_src

* markdown gfm live preview

- vmd mode requires atom shell
- This approach uses pandoc and [[https://github.com/netguy204/imp.el][impatient-mode]]
- TODO: This could be much better written using Node
...
If prefix arg ARG is passed, use the default browser."
  (interactive "P")
  (let ((browse-url-browser-function (if arg #'browse-url-default-browser #'xwwp)))
    (unless (process-status "httpd")
      (httpd-start))
    (impatient-mode 1)
    (imp-set-user-filter #'markdown-gfm-live-preview--filter)
    ;; works around a xwwp issue where the webkit buffer replaces the current buffer
    (unless arg
      (split-window-right)
      (other-window 1))
    (imp-visit-buffer)
    ;; works around a xwwp issue where the cursor seems like it's stuck in the webkit buffer.
    (unless arg
      (bury-buffer))))
#+end_src
...
(add-hook! ruby-mode #'set-ruby-keymaps)
#+end_src

#+begin_src elisp
;; clear projectile-rails's keybinds
(defun set-rails-keymaps ()
...
(add-hook! ruby-mode #'set-rails-keymaps)
#+end_src
...
Ensure test compilation buffers are selected and readonly.
...
(add-hook! ruby-mode #'set-testing-keybindings)
#+end_src
