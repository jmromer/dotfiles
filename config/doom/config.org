...
(when (display-graphic-p)
  (defun +config:darken-background ()
    (let* ((default-bg (face-background 'default))
           (darker-bg (doom-darken default-bg 0.25))) ; 25% darker
      (set-face-background 'default darker-bg)))
  (+config:darken-background) ;; Apply immediately
  (add-hook! doom-load-theme #'+config:darken-background) ;; Also apply after theme changes
nil)
#+end_src
...
(defun +disable-word-wrap ()
  (+word-wrap-mode -1))
(add-hook! org-mode #'+disable-word-wrap)
#+end_src
